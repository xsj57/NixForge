...
2. Add the following section in `configuration.nix`, change `/mnt/share` to the folder location in NixOS.:
```
{ config, pkgs, ... }:
{
  ...
  ...
  description = "Fix permissions on /mnt/share recursively";
    after = [ "mnt-share.mount" ];  # Run after mount
...
{ config, pkgs, ... }:
{
  ...
  environment.systemPackages = [
    ...
    _7zz    # Basic 7z support (if _7zz-rar does not work with override, use this instead)
    jq      # JSON processing
